...
ssh ${NAS_USER}@${NAS_IP} "$NAS_ENV; docker --version; docker compose version"

Write-Host ""
Write-Host "[3/6] Backup databázy pred aktualizáciou..." -ForegroundColor Yellow
ssh ${NAS_USER}@${NAS_IP} "$NAS_ENV; cd ${NAS_PATH}; docker compose exec -T web sh -c 'mkdir -p /data/backups && if [ -f /data/db.sqlite3 ]; then cp /data/db.sqlite3 /data/backups/db_`$(date +%Y%m%d_%H%M%S).sqlite3 && echo Backup hotovy; else echo Databaza este neexistuje; fi' || echo 'Backup preskočený (prvé nasadenie alebo kontajner nebeží)'"

Write-Host ""
Write-Host "[4/6] Zastavenie starých kontajnerov..." -ForegroundColor Yellow
ssh ${NAS_USER}@${NAS_IP} "$NAS_ENV; cd ${NAS_PATH}; docker compose down 2>/dev/null || true"

Write-Host ""
Write-Host "[5/6] Docker build a štart..." -ForegroundColor Yellow
ssh ${NAS_USER}@${NAS_IP} "$NAS_ENV; cd ${NAS_PATH}; docker compose build && docker compose up -d"

Write-Host ""
Write-Host "[6/6] Kontrola stavu kontajnera..." -ForegroundColor Yellow
ssh ${NAS_USER}@${NAS_IP} "$NAS_ENV; cd ${NAS_PATH}; docker compose ps; echo ''; docker compose logs --tail=20 web"
